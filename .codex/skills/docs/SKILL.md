...
cat .codex/specs/groups/<spec-group-id>/atomic/as-001-*.md
...
1. **Read architecture.yaml**: If `.codex/docs/structured/architecture.yaml` exists, read it to get the list of modules and their `path` globs.
...
4. **Check for doc updates**: For each matched module, check if `architecture.yaml` or any relevant flow files (in `.codex/docs/structured/flows/`) were also modified in this session.
...
Run `node .codex/scripts/docs-validate.mjs` to check for cross-reference issues.
```

### When to Skip

- If `.codex/docs/structured/architecture.yaml` does not exist (structured docs not adopted)
