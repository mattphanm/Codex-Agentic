...
cat .codex/specs/groups/<master-spec-group-id>/manifest.json
cat .codex/specs/groups/<master-spec-group-id>/spec.md

# List workstream spec groups
ls .codex/specs/groups/<master-spec-group-id>/workstreams/
```
...
Task({
  ...
  prompt: `
Analyze the MasterSpec spec group at .codex/specs/groups/<master-spec-group-id>/ and determine worktree allocation.
...
node .codex/scripts/session-checkpoint.mjs start-work <master-spec-id> orchestrator "Orchestrating: <objective>"

# Transition to implementing phase
node .codex/scripts/session-checkpoint.mjs transition-phase implementing
```
...
**Spec Group**: .codex/specs/groups/<master-spec-group-id>/workstreams/ws-1/
...
**Spec Group**: .codex/specs/groups/<master-spec-group-id>/workstreams/ws-1/
**Atomic Specs**: .codex/specs/groups/<master-spec-group-id>/workstreams/ws-1/atomic/
...
const manifest = JSON.parse(
  fs.readFileSync('.codex/specs/groups/<id>/workstreams/ws-1/manifest.json'),
);
...
3. **Spec Group Location**: .codex/specs/groups/<master-spec-group-id>/workstreams/ws-1/
...
node .codex/scripts/session-checkpoint.mjs dispatch-subagent challenge-pre-orch challenger "Pre-orchestration feasibility check" --stage pre-orchestration
node .codex/scripts/session-checkpoint.mjs transition-phase challenging
# Dispatch challenger subagent with stage: pre-orchestration
# After challenger completes:
node .codex/scripts/session-checkpoint.mjs complete-subagent challenge-pre-orch "Pre-orchestration challenge passed"

# Stage 2: Pre-test (after implementation, before test verification)
node .codex/scripts/session-checkpoint.mjs dispatch-subagent challenge-pre-test challenger "Pre-test feasibility check" --stage pre-test
node .codex/scripts/session-checkpoint.mjs transition-phase challenging
# After challenger completes:
node .codex/scripts/session-checkpoint.mjs complete-subagent challenge-pre-test "Pre-test challenge passed"

# Stage 3: Pre-review (after unify, before code review)
node .codex/scripts/session-checkpoint.mjs dispatch-subagent challenge-pre-review challenger "Pre-review feasibility check" --stage pre-review
node .codex/scripts/session-checkpoint.mjs transition-phase challenging
# After challenger completes:
node .codex/scripts/session-checkpoint.mjs complete-subagent challenge-pre-review "Pre-review challenge passed"
```
...
1. `.codex/context/session.json` - Global session state with phase, active work, worktree allocation
...
node .codex/scripts/session-checkpoint.mjs complete-subagent <task_id> "<summary>"

# After merge
node .codex/scripts/session-checkpoint.mjs transition-phase verifying
```
...
node .codex/scripts/session-checkpoint.mjs complete-work
```
